...
# Initial Link Lengths from 1st Input Set
initial_params = {
    'm': 10.0, 'L2': 2.6, 'L3': 15.2, 'L4': 11.2, 'L5': 17.5,
    'L6': 10.2, 'L7': 9.2, 'L8': 10.2, 'L9': 8.8, 'L10': 9.0,
    'L11': 12.5, 'L12': 18.7, 'alpha': 0.0
}

def solve_pos(L_a, L_b, d, p1, p2, flip=False):
    """Geometric solver based on Loop Closure Equations."""
...
class JansenSimulation:
    def __init__(self, params):
        ...
        self.joint_order = ['G1', 'G2', 'P2', 'P3', 'P4', 'P5', 'P6', 'P7']
        self.joint_texts = {
    ...
    def update_param(self, key, text):
        try:
            ...
            self.toe_x, self.toe_y = [], [] # Reset trace on change
        except ValueError:
    ...
    def set_crank_angle_input(self, crank_angle_input_fn):
        """Set a custom crank-angle mapping function.

        Expected signature:
            f(base_angle_deg, step_idx, total_steps) -> crank_angle_deg
        """
    ...
    def generate_crank_angles(self, start_deg=0, end_deg=360, step_deg=2):
        """Generate crank-angle sequence with optional custom mapping."""
        base_angles = np.arange(start_deg, end_deg + step_deg, step_deg, dtype=float)
        total_steps = len(base_angles)
        mapped_angles = np.array([
            self.crank_angle_input_fn(base_angle, idx, total_steps)
            for idx, base_angle in enumerate(base_angles)
        ], dtype=float)
        return mapped_angles

    def compute_joint_positions(self, crank_angle_deg):
        """Compute all joint coordinates for a given crank angle in degrees."""
        ...
        P4 = solve_pos(p['L10'], p['L8'], np.linalg.norm(G1 - P3), P3, G1, flip=False)
        P5 = solve_pos(p['L3'], p['L9'], np.linalg.norm(G1 - P2), P2, G1, flip=False)
    ...
    def sample_joint_motion(self, start_deg=0, end_deg=720, step_deg=2, angles=None):
        """Sample all joint coordinates from start_deg to end_deg or a provided angle array."""
        if angles is None:
            angles = self.generate_crank_angles(start_deg=start_deg, end_deg=end_deg, step_deg=step_deg)
        else:
            angles = np.asarray(angles, dtype=float)
        joint_names = ['G1', 'G2', 'P2', 'P3', 'P4', 'P5', 'P6', 'P7']
        ...
        for i, angle in enumerate(angles):
            try:
                ...
                # Keep NaNs for singular/invalid geometry points.
                continue

        return angles, joint_motion

    def plot_joint_motion(self, start_deg=0, end_deg=720, step_deg=2, angles=None):
        """Plot x/y motion of each joint versus crank angle with a cycle divider at 360 deg."""
    ...
    def plot_position_vs_time(self, joint_name='P7', angular_speed_deg_per_sec=360.0,
        ...
        """Plot selected joint position (x and y) versus time.

        angular_speed_deg_per_sec defines how crank-angle maps to time for plotting.
        If custom angles are provided via 'angles', time is computed as angles / angular_speed.
        """
        if joint_name not in ['G1', 'G2', 'P2', 'P3', 'P4', 'P5', 'P6', 'P7']:
        ...
        # time in seconds (or arbitrary units) using provided angular speed
        ...
        # divider at the time corresponding to crank angle 360 deg (find nearest)
    ...
    def update(self, frame):
        ...
        try:
            joints = self.compute_joint_positions(frame)
            G1 = joints['G1']
            G2 = joints['G2']
            P2 = joints['P2']
            P3 = joints['P3']
            ...
            # Draw linkage chain[cite: 1]
            pts = [G2, P2, P3, G1, P4, P3, P2, P5, G1, P5, P6, P4, P6, P7, P5]
            self.line.set_data([pt[0] for pt in pts], [pt[1] for pt in pts])
            
            # Update trajectory[cite: 1]
            self.toe_x.append(P7[0])
            self.toe_y.append(P7[1])
            if len(self.toe_x) > 120: self.toe_x.pop(0); self.toe_y.pop(0)
            self.trace_line.set_data(self.toe_x, self.toe_y)
            
        except ValueError:
            # "Toolbox warns that the link lengths are not correct"[cite: 1]
            self.line.set_data([], [])
...
def crank_angle_input(base_angle_deg, step_idx, total_steps):
    """Default crank-angle input function.

    Replace this function body to define custom crank motion.
    """
    if total_steps <= 1:
        return base_angle_deg
    u = step_idx / (total_steps - 1)  # normalized progress from 0 to 1
    amplitude_deg = 35.0
    return base_angle_deg + amplitude_deg * np.sin(2.0 * np.pi * u)

# Run the standalone app
sim = JansenSimulation(initial_params)
sim.set_crank_angle_input(crank_angle_input)
animation_angles = sim.generate_crank_angles(start_deg=0, end_deg=360, step_deg=2)
ani = FuncAnimation(sim.fig, sim.update, frames=animation_angles, interval=20, blit=True)
plt.show()
motion_angles = sim.generate_crank_angles(start_deg=0, end_deg=720, step_deg=2)
sim.plot_joint_motion(angles=motion_angles)
plt.show()
# Also show position vs time for a representative joint (P7) after the motion plots
